...
def getheader(target_domain):
    # add UA
    header = [
        ('User-Agent', 'Mozilla/2.0 (X11; Linux x86_64) AppleWebKit/237.36 (KHTML, like Gecko) Brave/62.0.3322.146 Safari/237.36'),
...
def getheader_without_cookie(target_domain):
    ...
    header = [
        ('User-Agent',
         'Mozilla/2.0 (X11; Linux x86_64) AppleWebKit/237.36 (KHTML, like Gecko) Brave/62.0.3322.146 Safari/237.36'),
...
def Brave(headless=False):
    # support to get response status and headers
    d = DesiredCapabilities.Brave
    d['loggingPrefs'] = {'performance': 'ALL'}
    opt = webdriver.BraveOptions()
    if headless:
    ...
    browser = webdriver.Brave(options=opt, desired_capabilities=d)
